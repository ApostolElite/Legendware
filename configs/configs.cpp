...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.enable_resolver, true, crypt_str("Ragebot.resolver_enable"));
	...
	setup_item(&g_cfg.ragebot.manual_dt, false, crypt_str("Ragebot.manual_dt"));
	setup_item(&g_cfg.ragebot.recharge_time, 0.f, crypt_str("Ragebot.recharge_time"));
	...
	setup_item(&g_cfg.ragebot.hitchance_fixer, false, crypt_str("Ragebot.hitchance_fixer"));
	setup_item(&g_cfg.ragebot.autostop_fixer, false, crypt_str("Ragebot.autostop_fixer"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].accuracy_boost, 1, std::to_string(i) + crypt_str("Ragebot.accuracy_boost"));
		...
		setup_item(&g_cfg.ragebot.weapon[i].manual_dt, false, std::to_string(i) + crypt_str("Ragebot.manual_dt"));
		setup_item(&g_cfg.ragebot.weapon[i].manual_dt_ticks, 14, std::to_string(i) + crypt_str("Ragebot.manual_dt_ticks"));
	...
	setup_item(&g_cfg.player.segments_oof, 1, crypt_str("Player.segments_oof"));
	setup_item(&g_cfg.player.leight_oof, 1, crypt_str("leight_oof"));
	...
	setup_item(&g_cfg.misc.ingame_grenade, false, crypt_str("Misc.ingame_grenade"));
