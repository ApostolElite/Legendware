...
{
	ImGui::StyleColorsClassic(); // colors setup настройка цвета меню!!!
...
void lua_edit(std::string window_name)
{
	...
	ImGui::Begin(window_name.c_str(), nullptr /*ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse */);//начало меню
	...
	if (!rage_section)
	{
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Enable resolver"), &g_cfg.ragebot.enable_resolver);
				...
				ImGui::Checkbox(crypt_str("Auto hitchance"), &g_cfg.ragebot.hitchance_fixer);
				ImGui::Checkbox(crypt_str("Autostop alternative"), &g_cfg.ragebot.autostop_fixer);

				ImGui::Checkbox(crypt_str("fire delay"), &g_cfg.ragebot.fire_delay);
				if(g_cfg.ragebot.fire_delay)
				{
					//ImGui::SliderFloat(crypt_str("delay time"), &g_cfg.ragebot.fire_delay_time, 0.01f, 1.f, crypt_str("%.1f"));

					ImGui::SliderInt(crypt_str("Automatic fire delay"), &g_cfg.ragebot.fire_delay_time, 0, 14, false, (!g_cfg.ragebot.fire_delay_time ? crypt_str("None") : (g_cfg.ragebot.fire_delay_time == 1 ? crypt_str("%d tick") : crypt_str("%d ticks"))));
				}
				...
				ImGui::Checkbox(crypt_str("Manual DT"), &g_cfg.ragebot.manual_dt);

				if (g_cfg.ragebot.manual_dt)
				{
					ImGui::SliderFloat(crypt_str("Recharge time"), &g_cfg.ragebot.recharge_time, 0.0f, 3.f, crypt_str("%.1f"));
				}
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Manual DT"), &g_cfg.ragebot.weapon[hooks::rage_weapon].manual_dt);

				if (g_cfg.ragebot.weapon[hooks::rage_weapon].manual_dt)
				{
					ImGui::SliderInt(crypt_str("Ticks"), &g_cfg.ragebot.weapon[hooks::rage_weapon].manual_dt_ticks, 0, 16);
				}
#endif
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (g_cfg.ragebot.weapon[hooks::rage_weapon].hitchance)
				{
					...
					ImGui::SliderInt(crypt_str("Accuracy boost"), &g_cfg.ragebot.weapon[hooks::rage_weapon].accuracy_boost, 1, 100);
				}
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			ImGui::Checkbox(crypt_str("Enable"), &g_cfg.player.enable);
////////////////////////////////////////////////////////////////////
			static const char* types_model[] = { "Test","Enforcer",
"Soldier",
"Ground Rebel",
"Maximus",
"Osiris",
...
"Agent Ava" };
			...
			draw_combo(crypt_str("Model type show"), model_type, types_model, ARRAYSIZE(types_model));
////////////////////////////////////////////////////////////////////
			if (player == ENEMY)
			{
				...
				if (g_cfg.player.arrows)
				{
					...
					ImGui::SliderInt(crypt_str("Arrows segments"), &g_cfg.player.segments_oof, 1, 100);
					ImGui::SliderInt(crypt_str("Arrows leight"), &g_cfg.player.leight_oof, 1, 100);
...
void c_menu::draw(bool is_open)
{
	...
	ImGui::Begin(crypt_str("legendware.pw v3"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		// крашит пизда PostProcessing::performFullscreenBlur(ImGui::GetWindowDrawList(), m_alpha / 1.3);
...
/*
	...
	//draw on player section or visual tab section
