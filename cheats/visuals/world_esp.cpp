...
void worldesp::molotov_timer(entity_t* entity) //индикатор молика
{
	...
	if (entity->GetClientClass()->m_ClassID == CInferno) {
		...
		render::get().Draw3DFilledCircle(entity->m_vecOrigin(), Vector(maxs - mins).Length2D() * 0.5, Color(120, 0, 0, g_cfg.esp.molotov_timer_color.a()));
