...
void misc::PovArrows(player_t* e, Color color)
{
	...
	float width = g_cfg.player.leight_oof;
	auto radius = g_cfg.player.distance;
	auto size = g_cfg.player.size;
	auto segments = g_cfg.player.segments_oof;

	render::get().arc_poly(Vector(wid / 2, heigth / 2, 0), radius + 6, DEG2RAD(angleYawRad - width), DEG2RAD(angleYawRad + width), segments, size, color);
	render::get().arc_poly(Vector(wid / 2, heigth / 2, 0), radius, DEG2RAD(angleYawRad - width), DEG2RAD(angleYawRad + width), segments, size, color);
}
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	if (g_ctx.globals.fakeducking && !g_cfg.ragebot.enable && g_cfg.ragebot.double_tap_key.key <= KEY_NONE || g_cfg.ragebot.double_tap_key.key >= KEY_MAX && !g_cfg.ragebot.double_tap && !g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->is_knife())
	{
	...
	{
		auto recharge_time = g_ctx.globals.weapon->can_double_tap() ? TIME_TO_TICKS(g_cfg.ragebot.recharge_time) : TIME_TO_TICKS(g_cfg.ragebot.recharge_time);
...
void misc::hide_shots(CUserCmd* m_pcmd, bool should_work)
{
	...
	g_ctx.globals.next_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 7 : 0;
	...
	if (g_cfg.misc.ingame_grenade)
	{
		for (int i = 0; i < cGrenade.GrenadeInfo.size(); i++) {
			...
			if (cGrenade.GetInfo(i, &info)) {
				/* Ваша отрисовка из параметров info */
				...
				if (dist < 2.0f) {
					...
					{
						if (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
						{
						...
						if (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_INCGRENADE)
						{
							render::get().text(fonts[GRENADES], vecAngles.x, vecAngles.y, Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, "l");
							render::get().circle(vecAngles.x, vecAngles.y, 15, 15, Color::Blue);
						}
						if (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_MOLOTOV)
						{
							render::get().text(fonts[GRENADES], vecAngles.x, vecAngles.y, Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, "k");
							render::get().circle(vecAngles.x, vecAngles.y, 15, 15, Color::Blue);
						}
						if (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE)
						{
							render::get().text(fonts[GRENADES], vecAngles.x, vecAngles.y, Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, "k");
							render::get().circle(vecAngles.x, vecAngles.y, 15, 15, Color::Blue);
						}
					...
					{
						auto box_width = render::get().text_width(fonts[DAMAGE_MARKER], info.szName.c_str());
						render::get().text(fonts[DAMAGE_MARKER], vecAngles.x + 120, vecAngles.y, Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, info.szName.c_str());
					}
					if (info.szDescription.c_str())
						render::get().text(fonts[DAMAGE_MARKER], iCenterX, iCenterY, Color::White, HFONT_CENTERED_NONE, info.szDescription.c_str());
				}
				else {
					if (math::world_to_screen(info.vecOrigin, vecOnScreenOrigin))
