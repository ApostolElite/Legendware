...
namespace math
{
	...
	void vector_angles(const Vector& forward, Vector& up, QAngle& angles)
	{
		...
		{
			angles.pitch = atan2f(-forward.z, forwardDist) * 180 / DirectX::XM_2PI;
			angles.yaw = atan2f(forward.y, forward.x) * 180 / DirectX::XM_2PI;

			float upZ = (left.y * forward.x) - (left.x * forward.y);
			angles.roll = atan2f(left.z, upZ) * 180 / DirectX::XM_2PI;
		}
		else
		{
			angles.pitch = atan2f(-forward.z, forwardDist) * 180 / DirectX::XM_2PI;
			angles.yaw = atan2f(-left.x, left.y) * 180 / DirectX::XM_2PI;
			angles.roll = 0;
		}
	}
	//--------------------------------------------------------------------------------
