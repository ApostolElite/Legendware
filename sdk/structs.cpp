...
void entity_t::set_abs_origin(const Vector& origin)
{
	...
	using Fn = void(__thiscall*)(void*, const Vector&);
	static auto fn = reinterpret_cast<Fn>(util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8")));// 55 8B EC 83 E4 F8 51 53 56 57 8B F1
...
int weapon_t::get_max_tickbase_shift()
{
	...
	{
		max_tickbase_shift = g_cfg.ragebot.weapon[hooks::rage_weapon].manual_dt_ticks;
	}
	else
	{
		switch (idx)
...
{
	return *(AnimationLayer**)((DWORD)this + 0x2990);//0x2980
}
...
{
	return *reinterpret_cast<c_baseplayeranimationstate**>(reinterpret_cast<void*>(uintptr_t(this) + 0x9960));//old  0x3914
}
